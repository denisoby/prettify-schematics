...
const overwriteFile: Rule = (tree: Tree, context: SchematicContext) => {
  ...
  const newContent = content!.toString().replace('enableProdMode();', `
    // enableProdMode here
    enableProdMode();
  `);
  debugger;
...
const modifyFile: Rule = (tree: Tree, context: SchematicContext) => {
  ...
  updateRecorder.insertRight(index + stringToSearch.length, ` /* here may be 'extends BlaBlaBla' */ `);
